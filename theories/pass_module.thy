...
proof -
  ...
  proof fix A
  show "\<forall>p. (card A \<ge> 1 \<and> finite_profile A p) \<longrightarrow> card (defer (Pass_module 1 r) A p) = 1"
  proof fix p show "(card A \<ge> 1 \<and> finite_profile A p) \<longrightarrow> card (defer (Pass_module 1 r) A p) = 1"
  proof assume A_valid: "(card A \<ge> 1 \<and> finite_profile A p)"
    hence finite_A: "finite A"
      by simp
    moreover have "A \<noteq> {}"
      using A_valid by auto
    moreover have lin_ord_on_A: "linear_order_on A (limit_to A r)" using order
      using limit_preserves_linear_order by blast
    ultimately have winner_exists: "\<exists>a\<in>A. above (limit_to A r) a = {a} \<and>
        (\<forall>x\<in>A. above (limit_to A r) x = {x} \<longrightarrow> x = a)"
      using linear_order_above_one_winner by (simp add: linear_order_above_one_winner)
    then obtain w where w_unique_top: "above (limit_to A r) w = {w} \<and>
        (\<forall>x\<in>A. above (limit_to A r) x = {x} \<longrightarrow> x = w)" using linear_order_above_one_winner
      by auto
    ...
    proof
      ...
      have "card(above (limit_to A r) w) \<le> 1" using w_top
        by auto
      hence "{w} \<subseteq> {a \<in> A. card(above (limit_to A r) a) \<le> 1}"
        using winner_exists w_unique_top by blast
      moreover have "{a \<in> A. card(above (limit_to A r) a) \<le> 1} \<subseteq> {w}"
      proof fix x assume x_in_winner_set: "x \<in> {a \<in> A. card (above (limit_to A r) a) \<le> 1}"
        hence x_in_A: "x \<in> A"
          by auto
        hence "(x,x) \<in> limit_to A r"
          by (meson lin_ord_on_A above_in_relation_equiv linear_order_impl_connex self_in_above)
        hence x_above_x: "x \<in> above (limit_to A r) x"
          by (simp add: above_def)
        have "above (limit_to A r) x \<subseteq> A" using limited_above_subset order
          by fastforce
        hence above_finite: "finite (above (limit_to A r) x)"
          by (simp add: A_valid finite_subset)
        have "card (above (limit_to A r) x) \<le> 1" using x_in_winner_set
          by simp
        moreover have "card (above (limit_to A r) x) \<ge> 1"
          by (metis One_nat_def Suc_leI above_finite card_eq_0_iff equals0D neq0_conv x_above_x)
  ...
  qed qed qed
  thus ?thesis
    using defers_def order pass_module_sound by blast
qed
...
proof -
  ...
  proof fix A
  show "\<forall>p. (card A \<ge> 2 \<and> finite_profile A p) \<longrightarrow> card (defer (Pass_module 2 r) A p) = 2"
  proof fix p show "(card A \<ge> 2 \<and> finite_profile A p) \<longrightarrow> card (defer (Pass_module 2 r) A p) = 2"
  proof assume A_ok: "card A \<ge> 2 \<and> finite_profile A p"
    hence finA: "finite A"
      by simp
    moreover from A_ok have not_empty_A: "A \<noteq> {}"
      by auto
    moreover have limitA_order: "linear_order_on A (limit_to A r)"
      using limit_preserves_linear_order order by auto
    ultimately obtain a where a: "above (limit_to A r) a = {a}"
      using linear_order_above_one_winner by blast
    ...
    from a have a_in_defer: "a \<in> defer (Pass_module 2 r) A p"
      by (metis (no_types, lifting) CollectI Pass_module.simps Suc_leI not_empty_A a_above
          card_UNIV_bool card_empty card_gt_0_iff card_insert_disjoint empty_iff empty_not_UNIV finA
          finite.emptyI finite_UNIV insert_iff limitA_order linear_order_above_one_winner sndI)
    have "finite (A-{a})"
      by (simp add: finA)
    moreover have A_not_only_a: "A-{a} \<noteq> {}"
      by (metis A_ok Diff_empty Diff_idemp Diff_insert0 One_nat_def not_empty_A card.insert_remove
          card_empty finite.emptyI insert_Diff numeral_le_one_iff semiring_norm(69))
    moreover have limitAa_order: "linear_order_on (A-{a}) (limit_to (A-{a}) r)"
      using limit_preserves_linear_order order by blast
    ultimately obtain b where b: "above (limit_to (A-{a}) r) b = {b}"
      using linear_order_above_one_winner by blast
    ...
    hence c_not_above_b: "\<forall>c \<in> A-{a, b}. c \<notin> above (limit_to A r) b"
      by (smt DiffD2 a_best above_def b insertCI insertE insert_Diff limit_preserves_preferences1
          limit_preserves_preferences2 limit_to_limits limited_dest mem_Collect_eq)
    ...
    moreover have b_above_b: "b \<in> above (limit_to A r) b"
      by (metis above_def b b_best limited_above_subset mem_Collect_eq order rev_subsetD singletonI)
    ultimately have above_b_eq_ab: "above (limit_to A r) b = {a, b}"
      using a_above by auto
    hence card_above_b_eq_2: "card (above (limit_to A r) b) = 2"
      using A_not_only_a b_in_limit by auto
    hence b_in_defer: "b \<in> defer (Pass_module 2 r) A p"
      using b_above_b above_subset by auto
    from b_best have b_above: "\<forall>c \<in> A-{a}. b \<in> above (limit_to A r) c"
      by (meson above_in_relation_equiv)
    have "connex_on A (limit_to A r)"
      using limitA_order linear_order_impl_connex by auto
    ...
    ultimately have "defer (Pass_module 2 r) A p \<subseteq> {a, b}"
      by blast
    from this a_in_defer b_in_defer have "defer (Pass_module 2 r) A p = {a, b}"
      by blast
    thus "card (defer (Pass_module 2 r) A p) = 2"
      using above_b_eq_ab card_above_b_eq_2 by auto
  qed qed qed
  thus ?thesis
    using defers_def order pass_module_sound by blast
qed
...
proof -
  ...
  proof fix A show "\<forall>p. (A \<noteq> {} \<and> finite_profile A p) \<longrightarrow> reject (Pass_module n r) A p \<noteq> A"
  proof fix p show "(A \<noteq> {} \<and> finite_profile A p) \<longrightarrow> reject (Pass_module n r) A p \<noteq> A"
  proof assume input_fine: "A \<noteq> {} \<and> finite_profile A p"
    hence "finite A"
      by simp
    moreover have "A \<noteq> {}"
    ...
    moreover have "linear_order_on A (limit_to A r)"
      using limit_preserves_linear_order order by auto
    ultimately have "\<exists>a\<in>A. above (limit_to A r) a = {a} \<and>
        (\<forall>x\<in>A. above (limit_to A r) x = {x} \<longrightarrow> x = a)" using linear_order_above_one_winner
      by (simp add: linear_order_above_one_winner)
    thus "reject (Pass_module n r) A p \<noteq> A"
      by (metis (no_types, lifting) One_nat_def Pass_module.simps Suc_leI assms(2) fst_conv
          is_singletonI is_singleton_altdef leD mem_Collect_eq snd_conv)
  qed qed qed
  thus ?thesis
