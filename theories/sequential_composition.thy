...
proof -
  ...
  from 0 1
  have "(elect m A p \<union> elect n ?new_A ?new_p)
          \<union> (reject m A p \<union> reject n ?new_A ?new_p)
          \<union> defer n ?new_A ?new_p = A"
    by blast
...
next
  ...
  from ele rej def n_electing_m profile
  ...
  from this def_card def electing_n n_electing_m profile
  ...
  from this def def_card electing_n n_electing_m profile res_m
  show ?thesis
...
lemma seq_comp_elect_set_passing:
  ...
  shows "elect (m \<triangleright> n) A p
          = elect n (defer m A p) (limit_profile_to (defer m A p) p) \<union> (elect m A p)"
...
proof -
  have "\<forall>A p. finite_profile A p \<longrightarrow> partition_of A (n A p)"
    ...
    by auto
  hence "finite_profile (defer m A p) (limit_profile_to (defer m A p) p) \<longrightarrow>
          partition_of (defer m A p) (n (defer m A p) (limit_profile_to (defer m A p) p))"
    by simp
  hence "partition_of (defer m A p) (n (defer m A p) (limit_profile_to (defer m A p) p))"
    by (meson assms(1) assms(3) seq_n_input_sound)
  thus ?thesis
    using assms(1) assms(2) assms(3) seq_comp_does_not_grow_deferred_set2
    by blast
qed
...
proof -
  ...
  from this non_blocking_m
  have 0: "?input_sound \<longrightarrow> A - reject m A p \<noteq> {}"
    by (meson Diff_eq_empty_iff non_blocking_def reject_from_input subset_antisym)
  from non_blocking_m
  have "?input_sound \<longrightarrow> partition_of A (m A p)"
  ...
  hence "?input_sound \<longrightarrow> (elect m A p \<noteq> {} \<or> defer m A p \<noteq> {})"
    by simp
  from this non_blocking_m non_blocking_n
  show ?thesis
...
proof -
  { have "\<forall>A p q w. (finite A \<and> w \<in> elect (m \<triangleright> n) A p \<and> lifted A p q w) \<longrightarrow> w \<in> elect (m \<triangleright> n) A q"
    proof
      { fix A show "\<forall>p q w. (finite A \<and> w \<in> elect (m \<triangleright> n) A p \<and> lifted A p q w) \<longrightarrow>
                      w \<in> elect (m \<triangleright> n) A q"
        proof
          { fix p show "\<forall>q w. (finite A \<and> w \<in> elect (m \<triangleright> n) A p \<and> lifted A p q w) \<longrightarrow>
                                w \<in> elect (m \<triangleright> n) A q"
            proof
              { fix q show "\<forall>w. (finite A \<and> w \<in> elect (m \<triangleright> n) A p \<and> lifted A p q w) \<longrightarrow>
                                  w \<in> elect (m \<triangleright> n) A q"
                proof
                  { fix w show "(finite A \<and> w \<in> elect (m \<triangleright> n) A p \<and> lifted A p q w) \<longrightarrow>
                                  w \<in> elect (m \<triangleright> n) A q"
                    proof
                      assume a: "finite A \<and> w \<in> elect (m \<triangleright> n) A p \<and> lifted A p q w"
                      hence profiles: "finite_profile A p \<and> finite_profile A q"
                        by (metis (no_types) lifted_def)
                      from this a def_monotone_m non_ele_m def_one_m electing_n
    ...
    qed }
  thus ?thesis
    by (metis (no_types, lifting) electing_def electing_n electoral_modules.monotone_def
        non_ele_m non_electing_def seq_creates_modules)
qed
...
proof -
  ...
  next
    ...
    have "finite_profile A q"
      by (simp add: lifted_def)
    from this modules new_A_eq
    have 1: "finite_profile ?new_Ap ?new_q"
      by (metis (no_types) seq_n_input_sound)
    moreover from modules profile_p input_ok
    have 0: "finite_profile ?new_Ap ?new_p"
      by (metis (no_types) seq_n_input_sound)
    moreover from defer_subset input_ok
    have 2: "a \<in> ?new_Ap"
      by blast
    moreover from input_ok
    have eql_lengths: "size ?new_p = size ?new_q"
      by (simp add: lifted_def)
    ...
    from this input_ok modules mono_m
    have "\<forall>i. (0 \<le> i \<and> i < size ?new_p) \<longrightarrow>
    ...
    from this 0 eql_lengths mono_m
    show ?thesis
...
proof -
  { have "\<forall>A p. (card A \<ge> 1 \<and> finite_profile A p) \<longrightarrow> card (defer (m \<triangleright> n) A p) = 1"
    proof
      { fix A show "\<forall> p. (card A \<ge> 1 \<and> finite_profile A p) \<longrightarrow> card (defer (m \<triangleright> n) A p) = 1"
        proof
          { fix p show "(card A \<ge> 1 \<and> finite_profile A p) \<longrightarrow> card (defer (m \<triangleright> n) A p) = 1"
            proof
              assume input_fine: "(card A \<ge> 1 \<and> finite_profile A p)"
              hence "A \<noteq> {}"
                by auto
              moreover from input_fine
              have finite_profile: "finite_profile A p"
                by simp
              ...
              moreover have "elect m A p = {}"
                ...
                using finite_profile non_electing_def
                by auto
              ultimately have "defer m A p \<noteq> {}"
              proof - (* generated proof*)
                have "\<exists>a. a \<in> A \<and> a \<notin> reject m A p"
                  by (meson \<open>reject m A p \<noteq> A\<close> input_fine non_electing_def non_electing_m
                      reject_from_input subset_antisym subset_iff)
                then show ?thesis
                  by (metis (no_types) electoral_module_element_in_defer empty_iff input_fine
                      non_electing_def non_electing_m)
              qed
              hence "card (defer m A p) \<ge> 1"
                by (metis One_nat_def Suc_leI card_gt_0_iff input_fine non_blocking_def
                    non_blocking_m seq_n_input_sound)
                { thus "card (defer (m \<triangleright> n) A p) = 1"
                  proof - (*generated proof*)
                    have f1: "(elect (m \<triangleright> n) A p, snd ((m \<triangleright> n) A p))
                        = (elect m A p \<union> elect n (defer m A p) (limit_profile_to (defer m A p) p),
                          reject m A p \<union> reject n (defer m A p) (limit_profile_to (defer m A p) p),
                          defer n (defer m A p) (limit_profile_to (defer m A p) p))"
                      by (metis prod.collapse seq_comp.simps)
                    have "\<forall>n f. defers n f
                         = (electoral_module f
                            \<and> (\<forall>A rs. (\<not> n \<le> card (A::'a set) \<or> infinite A \<or> \<not> profile_on A rs)
                                      \<or> card (defer f A rs) = n))"
                      using defers_def
                      by blast
                    hence "card (defer n (defer m A p) (limit_profile_to (defer m A p) p)) = 1"
                      by (meson \<open>1 \<le> card (defer m A p)\<close> assms(3) finite_profile non_blocking_def
                          non_blocking_m seq_n_input_sound)
                    then show ?thesis
                      using f1
                      by fastforce
...
proof -
  ...
  moreover
    { have "\<forall>S. finite S \<longrightarrow>
            (\<exists>A \<subseteq> S.
              (\<forall>a \<in> A. independent_of (m \<triangleright> m2) S a \<and>
                (\<forall>p. finite_profile S p \<longrightarrow> a \<in> reject (m \<triangleright> m2) S p))
              \<and> (\<forall>a \<in> S-A. independent_of n S a \<and> (\<forall>p. finite_profile S p \<longrightarrow> a \<in> reject n S p)))"
      proof
        { fix S show "finite S \<longrightarrow>
            (\<exists>A \<subseteq> S.
              (\<forall>a \<in> A. independent_of (m \<triangleright> m2) S a
                \<and> (\<forall>p. finite_profile S p \<longrightarrow> a \<in> reject (m \<triangleright> m2) S p))
              \<and> (\<forall>a \<in> S-A. independent_of n S a \<and> (\<forall>p. finite_profile S p \<longrightarrow> a \<in> reject n S p)))"
          proof
            { assume fin: "finite S"
              obtain A where
                A: "A \<subseteq> S \<and>
                  (\<forall>a \<in> A. independent_of m S a
                  \<and> (\<forall>p. finite_profile S p \<longrightarrow> a \<in> reject m S p))
                  \<and> (\<forall>a \<in> S-A. independent_of n S a
                    \<and> (\<forall>p. finite_profile S p \<longrightarrow> a \<in> reject n S p))"
                by (metis (no_types, lifting) compatible disjoint_compatible_def fin)
              show "\<exists>A. A \<subseteq> S
                    \<and> (\<forall>a \<in> A. independent_of (m \<triangleright> m2) S a
                      \<and> (\<forall>p. finite_profile S p \<longrightarrow> a \<in> reject (m \<triangleright> m2) S p))
                    \<and> (\<forall>a \<in> S-A. independent_of n S a
                      \<and> (\<forall>p. finite_profile S p \<longrightarrow> a \<in> reject n S p))"
              proof
                { have "\<forall>a p q. a \<in> A \<and> only_change S p q a \<longrightarrow> (m \<triangleright> m2) S p = (m \<triangleright> m2) S q"
                  proof
                    { fix a show "\<forall>p q. a \<in> A \<and> only_change S p q a \<longrightarrow> (m \<triangleright> m2) S p = (m \<triangleright> m2) S q"
                      proof
                        { fix p show "\<forall>q. a \<in> A \<and> only_change S p q a \<longrightarrow>
                                        (m \<triangleright> m2) S p = (m \<triangleright> m2) S q"
                          proof
                            { fix q show "a \<in> A \<and> only_change S p q a \<longrightarrow>
                                            (m \<triangleright> m2) S p = (m \<triangleright> m2) S q"
                              proof
                                assume a: "a \<in> A \<and> only_change S p q a"
                                ...
                                have profiles: "finite_profile S p \<and> finite_profile S q"
                                  using only_change_def
                                  by fastforce
                                hence "(defer m S p) \<subseteq> S"
                                  using compatible defer_from_input disjoint_compatible_def
                                  by blast
                                hence "limit_profile_to (defer m S p) p
                                        = limit_profile_to (defer m S q) q"
                                  by (metis (no_types, lifting) A DiffD2 a compatible
                                      defer_alternative_representation disjoint_compatible_def
                                      eq_def profiles remove_only_difference_profile)
                                from this eq_def
                                have "m2 (defer m S p) (limit_profile_to (defer m S p) p) =
...
proof -
  ...
  have electoral_module_m: "electoral_module m"
    using strong_def_mon_m defer_invariant_monotone_def
    by auto
    ...
    { have "\<forall>A p q a. (a \<in> (defer (m \<triangleright> n) A p) \<and> lifted A p q a) \<longrightarrow> (m \<triangleright> n) A p = (m \<triangleright> n) A q"
      proof
        { fix A
          ...
          proof
            { fix p
              show "\<forall> q a. (a \<in> (defer (m \<triangleright> n) A p) \<and> lifted A p q a) \<longrightarrow> (m \<triangleright> n) A p = (m \<triangleright> n) A q"
              proof
                { fix q show "\<forall> a. (a \<in> (defer (m \<triangleright> n) A p) \<and> lifted A p q a) \<longrightarrow>
                                            (m \<triangleright> n) A p = (m \<triangleright> n) A q"
                  proof
                    { fix a show "(a \<in> (defer (m \<triangleright> n) A p) \<and> lifted A p q a) \<longrightarrow>
                                          (m \<triangleright> n) A p = (m \<triangleright> n) A q"
                      proof cases
                        ...
                        have finite_profile_p: "finite_profile A p"
                          ...
                          { thus ?thesis
                            ...
                            next
                              ...
                              hence input_valid:
                                  "(a \<in> defer (m \<triangleright> n) A p \<and> electoral_modules.lifted A p q a)"
                                by simp
                                { thus ?thesis
                                  proof cases
                                    ...
                                    hence a_single_defer: "{a} = defer m A q"
                                      ...
                                      by (metis electoral_module_n finite_profile_p input_valid
                                          seq_defer_in_both_defer defer_invariant_monotone_def)
                                    moreover have "(finite_profile A q \<and> {a} = defer m A q) \<longrightarrow>
                                                      (defer (m \<triangleright> n) A q \<subseteq> {a})"
                                      using finite_profile_p electoral_module_m electoral_module_n
                                        deferred_alt_always_deferred non_electing_m
                                      by blast
                                    ultimately have
                                      "((a \<in> defer m A p) \<and> (lifted A p q a) \<and> finite_profile A q)
                                          \<longrightarrow> defer (m \<triangleright> n) A q \<subseteq> {a}"
                                      by blast
                                    ...
                                    ultimately have "((a \<in> defer m A p) \<and> (lifted A p q a)) \<longrightarrow>
                                      ...
                                      by blast
                                    moreover have "((a \<in> defer m A p) \<and> (lifted A p q a)) \<longrightarrow>
                                                    card (defer (m \<triangleright> n) A q) = 1"
                                      by (metis One_nat_def
                                          \<open>electoral_modules.lifted A p q a \<longrightarrow> finite_profile A q\<close>
                                          a_single_defer card_empty card_insert_disjoint defers_1
                                          defers_def electoral_module_m empty_iff finite.emptyI
                                          le_numeral_extra(4) seq_comp_defer_set_passing
                                          seq_n_input_sound) (*lifted defer set size 1*)
                                    moreover have "a \<in> (defer (m \<triangleright> n) A p) \<longrightarrow> (a \<in> defer m A p)"
                                      using electoral_module_m electoral_module_n
                                        seq_comp_does_not_grow_deferred_set2 finite_profile_p
                                      by blast
                                    ultimately have
                                      "(a \<in> (defer (m \<triangleright> n) A p) \<and> (lifted A p q a)) \<longrightarrow>
                                          defer (m \<triangleright> n) A q = {a}" (*lifted defer set = a*)
                                      by (metis Collect_mem_eq card_1_singletonE
                                          empty_Collect_eq insertCI subset_singletonD)
                                    moreover have "(a \<in> (defer (m \<triangleright> n) A p) \<and> (lifted A p q a)) \<longrightarrow>
                                      ...
                                      using defers_1
                                      by (smt Diff_insert_absorb One_nat_def Set.set_insert
                                          card_0_eq card_Diff_singleton card_insert_disjoint
                                          card_mono defers_def electoral_module_m empty_iff
                                          empty_subsetI finite.emptyI finite.insertI finite_Diff
                                          finite_profile_p insert_subset seq_comp_defer_set_passing
                                          seq_comp_does_not_grow_deferred_set2 seq_n_input_sound
                                          singletonI)
                                    ultimately have "(a \<in> defer (m \<triangleright> n) A p \<and> lifted A p q a) \<longrightarrow>
                                      ...
                                      by blast
                                    moreover have "lifted A p q a \<longrightarrow>
                                      elect (m \<triangleright> n) A p = elect (m \<triangleright> n) A q"
                                      (*elect sets sets equal*)
                                      using
                                        \<open>electoral_modules.lifted A p q a \<longrightarrow> finite_profile A q\<close>
                                        finite_profile_p non_electing_def non_electing_m
                                        non_electing_n seq_comp_preserves_non_electing
                                      by blast (* elect sets equal*)
                                    thus ?thesis
                                      by (meson
                                          \<open>electoral_modules.lifted A p q a \<longrightarrow> finite_profile A q\<close>
                                          calculation defer_and_elect_equal_impl_result_equal
                                          electoral_module_m electoral_module_n finite_profile_p
                                          seq_creates_modules)
                                  next
                                    assume not_different_alternatives:
                                      "\<not>\<not>(defer m A q = defer m A p)"
                                    have "elect m A p = {}"
                                      using non_electing_m lifted
                                      by (simp add: finite_profile_p non_electing_def)
                                    moreover have "elect m A q = {}"
                                      using non_electing_m lifted
                                      by (simp add: finite_profile_q non_electing_def)
                                    ...
                                    have same_alternatives: "defer m A q = defer m A p"
                                      by simp
                                    hence "(limit_profile_to (defer m A p) p)
                                            = (limit_profile_to (defer m A p) q)
                                          \<or> lifted (defer m A q) (limit_profile_to (defer m A p) p)
                                              (limit_profile_to (defer m A p) q) a"
                                      by (metis defer_from_input electoral_module_m finite_profile_p
                                          input_valid limit_lifted_profile_interaction)
                                      { thus ?thesis
                                        proof
                                          assume "limit_profile_to (defer m A p) p
                                                    = limit_profile_to (defer m A p) q"
                                          hence same_profile:
                                            "limit_profile_to (defer m A p) p
                                              = limit_profile_to (defer m A q) q"
                                            using same_alternatives
                                            by simp
                                          hence results_equal_n:
                                            "n (defer m A q) (limit_profile_to (defer m A q) q) =
                                          ...
                                          moreover have results_equal_m: "m A p = m A q"
                                            ...
                                            by (simp add: defer_and_elect_equal_impl_result_equal
                                                electoral_module_m finite_profile_p
                                                finite_profile_q)
                                          ultimately have "(m \<triangleright> n) A p = (m \<triangleright> n) A q"
                                            using same_profile
                                            by auto
                                          thus ?thesis
                                            by blast
                                        next
                                          ...
                                          hence a_in_def_p:
                                            ...
                                            by (meson electoral_module_m electoral_module_n
                                                finite_profile_p input_valid
                                          ...
                                          have card_le_1_p: "card (defer m A p) \<ge> 1"
                                            by (metis One_nat_def Suc_leI card_gt_0_iff
                                                electoral_module_m electoral_module_n equals0D
                                                finite_profile_p input_valid seq_defer_in_both_defer
                                                seq_n_input_sound)
                                          hence "card (defer n (defer m A p)
                                                  (limit_profile_to (defer m A p) p)) = 1"
                                            using defers_1
                                            by (meson defers_def electoral_module_m finite_profile_p
                                                seq_n_input_sound)
                                          hence def_set_is_a_p:
                                            "{a} = defer n (defer m A p)
                                                    (limit_profile_to (defer m A p) p)"
                                            using a_still_deferred_p
                                            by (metis card_1_singletonE insert_subset singletonD)
                                          have a_still_deferred_q:
                                            "a \<in> defer n (defer m A q)
                                              (limit_profile_to (defer m A p) q)"
                                            using still_lifted
                                            by (metis a_in_def_p defer_monotone_def defer_monotone_n
                                                electoral_module_m finite_profile_p
                                                same_alternatives seq_n_input_sound)
                                          have "card (defer m A q) \<ge> 1"
                                            using card_le_1_p same_alternatives
                                            by auto
                                          hence "card (defer n (defer m A q)
                                                  (limit_profile_to (defer m A q) q)) = 1"
                                            using defers_1
                                            by (meson defers_def electoral_module_m finite_profile_q
                                                seq_n_input_sound)
                                          hence def_set_is_a_q:
                                            "{a} = defer n (defer m A q)
                                              (limit_profile_to (defer m A q) q)"
                                            using a_still_deferred_q
                                            by (metis card_1_singletonE same_alternatives
                                                singletonD)
                                          have "defer n (defer m A p)
                                                  (limit_profile_to (defer m A p) p)
                                                = defer n (defer m A q)
                                                    (limit_profile_to (defer m A q) q)"
                                            ...
                                            by auto
                                          thus ?thesis
                                            by (metis defer_and_elect_equal_impl_result_equal
                                                elect_m_equal electoral_module_m finite_profile_p
                                                finite_profile_q non_electing_def non_electing_n
                                                seq_comp_defer_set_passing
                                                seq_comp_elect_set_passing seq_creates_modules
                                                seq_n_input_sound)
                                        qed }
                                    qed }
                                qed }
