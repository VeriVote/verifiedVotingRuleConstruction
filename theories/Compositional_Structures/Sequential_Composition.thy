...
lemma voters_determine_seq_comp:
  ...
  assumes "voters_determine_election m \<and> voters_determine_election n"
  shows "voters_determine_election (m \<triangleright> n)"
proof (unfold voters_determine_election.simps, clarify)
  fix
    A :: "'a set" and
    V :: "'v set" and
    p p' :: "('a, 'v) Profile"
  ...
  hence coincide_limit:
    "\<forall> v \<in> V. limit_profile (defer m V A p) p v =
                limit_profile (defer m V A p') p' v"
    using coincide
    by simp
  moreover have
    "elect m V A p
  ...
  moreover have
    "reject m V A p
      \<union> reject n V (defer m V A p) (limit_profile (defer m V A p) p) =
    reject m V A p'
      \<union> reject n V (defer m V A p') (limit_profile (defer m V A p') p')"
    using assms eq coincide_limit
    unfolding voters_determine_election.simps
...
proof -
  ...
  have disjoint_m: "disjoint3 (m V A p)"
    unfolding \<S>\<C>\<F>_result.electoral_module.simps well_formed_\<S>\<C>\<F>.simps
    by blast
  ...
  proof -
    ...
    then obtain g :: "'a set \<Rightarrow> 'a set \<Rightarrow> 'a" where
      "\<forall> B B'.
        (B \<inter> B' = {}
          \<longrightarrow> (\<forall> a b. a \<in> B \<and> b \<in> B' \<longrightarrow> a \<noteq> b)) \<and>
          (B \<inter> B' \<noteq> {}
            \<longrightarrow> f B B' \<in> B \<and> g B B' \<in> B' \<and> f B B' = g B B')"
      by auto
    thus ?thesis
      using defer_in_A disj_n module_n prof_def_lim prof
      by (metis (no_types, opaque_lifting))
  qed
  have rej_intersect_new_elect_empty:
    "reject m V A p \<inter> elect n V ?new_A ?new_p = {}"
  ...
  next
    ...
    assume
      "x \<in> elect n V (defer m V A p)
              (limit_profile (defer m V A p) p)" and
      "x \<in> reject m V A p"
    thus "x \<in> {}"
  ...
  next
    ...
    thus "x \<in> {}"
      using module_n prof_def_lim reject_not_elected_or_deferred
      by blast
  qed
  ultimately have
...
proof (unfold sequential_composition'.simps sequential_composition.simps)
  have "\<forall> m n V A E.
      (case m V A E of (e, r, d) \<Rightarrow>
        ...
        (e \<union> e', r \<union> r', d')) =
          (elect m V A E
            \<union> elect n V (defer m V A E) (limit_profile (defer m V A E) E),
            reject m V A E
            \<union> reject n V (defer m V A E) (limit_profile (defer m V A E) E),
  ...
  thus
    "(\<lambda> m n V A p.
      ...
      (elect m V A p \<union> elect n V A' p',
        reject m V A p \<union> reject n V A' p',
        defer n V A' p')) =
      (\<lambda> m n V A pr.
...
proof (unfold \<S>\<C>\<F>_result.electoral_module.simps, safe)
  ...
  ultimately show "well_formed_\<S>\<C>\<F> A ((m \<triangleright> n) V A p)"
    using assms seq_comp_presv_disj seq_comp_presv_alts
    by metis
qed

subsection \<open>Lemmas\<close>

lemma seq_comp_decrease_only_defer:
  fixes
    m n :: "('a, 'v, 'a Result) Electoral_Module" and
    A :: "'a set" and
    V :: "'v set" and
...
proof -
  ...
  next
    ...
    thus "snd ((m \<triangleright> n) V A p) = snd (m V A p)"
      unfolding sequential_composition.simps
      using rej_empty empty_defer module_n prof_no_alt prof sndI sup_bot_right
      by metis
  qed
qed
...
next
  ...
  have rej: "\<exists> a \<in> A. reject m V A p = A - {a}"
    using Diff_empty def_one_m f_prof reject_not_elected_or_deferred
    unfolding defers_def
    by metis
  from ele rej def n_electing_m f_prof
  have res_m: "\<exists> a \<in> A. m V A p = ({}, A - {a}, {a})"
    using Diff_empty elect_rej_def_combination reject_not_elected_or_deferred
    unfolding non_electing_def
    by metis
...
lemma seq_comp_defers_def_set:
  fixes
    ...
    V :: "'v set" and
    p :: "('a, 'v) Profile"
  shows "defer (m \<triangleright> n) V A p =
          defer n V (defer m V A p) (limit_profile (defer m V A p) p)"
  using snd_conv
  unfolding sequential_composition.simps
  by metis
...
lemma seq_comp_def_then_elect_elec_set:
  ...
  shows "elect (m \<triangleright> n) V A p =
            elect n V (defer m V A p)
              (limit_profile (defer m V A p) p) \<union> (elect m V A p)"
...
proof -
  ...
  have "?input_sound \<longrightarrow> well_formed_\<S>\<C>\<F> A (m V A p)"
    unfolding \<S>\<C>\<F>_result.electoral_module.simps non_blocking_def
    by simp
  ...
  proof (unfold non_blocking_def)
    assume
      emod_reject_m:
      "\<S>\<C>\<F>_result.electoral_module m
      \<and> (\<forall> A V p. A \<noteq> {} \<and> finite A \<and> profile V A p
          \<longrightarrow> reject m V A p \<noteq> A)" and
      emod_reject_n:
      "\<S>\<C>\<F>_result.electoral_module n
      \<and> (\<forall> A V p. A \<noteq> {} \<and> finite A \<and> profile V A p
          \<longrightarrow> reject n V A p \<noteq> A)"
    show
      "\<S>\<C>\<F>_result.electoral_module (m \<triangleright> n)
      \<and> (\<forall> A V p. A \<noteq> {} \<and> finite A \<and> profile V A p
          \<longrightarrow> reject (m \<triangleright> n) V A p \<noteq> A)"
    proof (safe)
      show "\<S>\<C>\<F>_result.electoral_module (m \<triangleright> n)"
        using emod_reject_m emod_reject_n seq_comp_sound
        by metis
    next
      fix
        A :: "'a set" and
      ...
      have fin_defer:
        ...
        \<and> profile V (defer m V A p) (limit_profile (defer m V A p) p)"
      ...
      have def_limit:
        "defer (m \<triangleright> n) V A p =
          defer n V (defer m V A p) (limit_profile (defer m V A p) p)"
        using seq_comp_defers_def_set
        by metis
      from emod_reject_n emod_reject_m fin_A prof_A
      have "elect (m \<triangleright> n) V A p \<union> defer (m \<triangleright> n) V A p =
              A - reject (m \<triangleright> n) V A p"
      ...
      have rej_def_eq_set:
        ...
        by (simp add: reject_not_elected_or_deferred)
      have
        "defer n V (defer m V A p) (limit_profile (defer m V A p) p) -
          defer n V (defer m V A p) (limit_profile (defer m V A p) p) = {} \<longrightarrow>
      ...
      thus "x \<in> {}"
        using rej_def_eq_set result_disj fin_defer Diff_cancel Diff_empty fin_A prof_A
              emod_reject_m emod_reject_n reject_not_elected_or_deferred x_in_A
        by metis
    qed
  qed
qed
...
proof -
  have defer_card_eq_one:
    "\<forall> A V p. (card A \<ge> 1 \<and> finite A \<and> profile V A p)
  ...
  proof -
    have "\<forall> m'.
          (\<not> electing m' \<or> \<S>\<C>\<F>_result.electoral_module m'
          \<and> (\<forall> A' V' p'. (A' \<noteq> {} \<and> finite A' \<and> profile V' A' p')
       ...
       "\<forall> m'::('a, 'v, 'a Result) Electoral_Module.
        (\<not> electing m' \<or> \<S>\<C>\<F>_result.electoral_module m' \<and>
          (\<forall> A' V' p'. (A' \<noteq> {} \<and> finite A' \<and> profile V' A' p')
            \<longrightarrow> elect m' V' A' p' \<noteq> {}))
        \<and> (electing m' \<or> \<not> \<S>\<C>\<F>_result.electoral_module m' \<or> A m' \<noteq> {}
        \<and> finite (A m') \<and> profile (V m') (A m') (p m')
    ...
    have def_card_one:
      "\<S>\<C>\<F>_result.electoral_module m
      \<and> (\<forall> A V p. (1 \<le> card A \<and> finite A \<and> profile V A p)
          \<longrightarrow> card (defer m V A p) = 1)"
...
lemma def_lift_inv_seq_comp_help:
  ...
  assumes
    ...
    voters_determine_n: "voters_determine_election n" and
...
proof -
  ...
  next
    ...
    with mono_m
    show ?thesis
      using leI not_less_zero nth_equalityI voters_determine_n
      unfolding voters_determine_election.simps
      by presburger
  qed
  from mono_m mono_n
...
theorem disj_compat_seq[simp]:
  fixes m m' n :: "('a, 'v, 'a Result) Electoral_Module"
  assumes
...
next
  ...
  proof
    ...
    proof (safe)
      ...
      hence eq_defer: "defer m V S p = defer m V S q"
        using rej_A
...
next
  ...
  hence diff_elect_m: "A - elect m V A p = A"
    ...
    by (metis (full_types))
  have cond_win:
    "finite A \<and> finite V \<and> profile V A p
  ...
  have nb_n_full:
    "\<S>\<C>\<F>_result.electoral_module n \<and>
      (\<forall> A' V' p'.
        A' \<noteq> {} \<and> finite A' \<and> finite V' \<and> profile V' A' p'
          \<longrightarrow> reject n V' A' p' \<noteq> A')"
  ...
  hence seq_rej_union_subset_A:
    "reject m V A p
    \<union> reject n V (defer m V A p) (limit_profile (defer m V A p) p) \<subseteq> A"
  ...
  hence "reject (m \<triangleright> n) V A p \<subseteq> A - {a}"
    using seq_rej_union_subset_A seq_snd_simplified set_ins def_seq_diff nb_n_full
          ...
          reject_not_elected_or_deferred seq_comp_def_then_elect_elec_set finite_subset
...
next
  ...
  hence a_in_rej_union:
    ...
    using Diff_iff a'_in_rej condorcet_winner.simps cw_a
          reject_not_elected_or_deferred sound_m
    by (metis (no_types))
  have m_seq_n_full:
...
next
  ...
  ultimately have "(m \<triangleright> n) V A p = ({}, A - {a}, {a})"
    using Diff_empty cw_a elect_rej_def_combination
          reject_not_elected_or_deferred sound_seq_m_n condorcet_winner.simps
    by (metis (no_types))
...
theorem def_inv_mono_imp_def_lift_inv[simp]:
  ...
  assumes
    ...
    voters_determine_n: "voters_determine_election n"
  shows "defer_lift_invariance (m \<triangleright> n)"
proof (unfold defer_lift_invariance_def, safe)
  have "\<S>\<C>\<F>_result.electoral_module m"
...
next
  ...
  hence n_defers_exactly_one_p: "card (defer n V A p) = 1"
    ...
    unfolding defers_def
    by (metis (no_types))
  have fin_prof_def_m_q:
    "profile V (defer m V A q) (limit_profile (defer m V A q) q)"
    using def_presv_prof electoral_mod_m finite_profile_q
    by (metis (no_types))
  have def_seq_m_n_q:
    "defer (m \<triangleright> n) V A q =
  ...
  hence "card (defer n V (defer n V (defer m V A p)
        (limit_profile (defer m V A p) p))
      (limit_profile (defer n V (defer m V A p)
        (limit_profile (defer m V A p) p))
  ...
  next
    ...
    ultimately have elect_m_equal:
      ...
      by simp
    have "(\<forall> v \<in> V. (limit_profile (defer m V A p) p) v =
                      (limit_profile (defer m V A p) q) v)
    ...
    moreover have
      ...
      using voters_determine_n def_eq
      unfolding voters_determine_election.simps
      by presburger
    moreover have
