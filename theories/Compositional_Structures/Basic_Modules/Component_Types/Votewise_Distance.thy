...
proof -
  ...
  also have "... = n (map (case_prod (\<lambda> x y. d (A, x) (A', y)))
                          (?listpi (zip p p')))"
    using permute_list_map[of \<phi> ?z ?c_prod] perm len_eq
    by (simp add: atLeast_upt)
  also have "... = n (map2 (\<lambda> x y. d (A, x) (A', y)) (?listpi p) (?listpi p'))"
    using len_eq perm
    by (simp add: atLeast_upt permute_list_zip)
  finally show ?thesis
    by simp
qed
...
proof -
  have "v < v' \<or> v' < v"
    using assms(3) linorder_less_linear 
    by blast
  hence "{x \<in> V. x < v} \<subset> {x \<in> V. x < v'} \<or> {x \<in> V. x < v'} \<subset> {x \<in> V. x < v}"
    using assms(1) assms(2) dual_order.strict_trans 
    by blast
  thus ?thesis
    by (metis (full_types) assms(1) assms(2) assms(3) assms(4) sorted_list_of_set_nth_equals_card)
qed
...
lemma permute_invariant_under_coinciding_funs:
  ...
  by (simp add: assms permute_list_def)
...
proof (unfold distance_anonymity_def, safe)
  ...
  proof -
    have rn_A_eq_A: "fst ?rn1 = A" by simp
    have rn_A'_eq_A': "fst ?rn2 = A'" by simp
    have rn_V_eq_pi_V: "fst (snd ?rn1) = ?rn_V" by simp
    have rn_V'_eq_pi_V': "fst (snd ?rn2) = ?rn_V'" by simp
    have rn_p_eq_pi_p: "snd (snd ?rn1) = ?rn_p" by simp
    have rn_p'_eq_pi_p': "snd (snd ?rn2) = ?rn_p'" by simp
    show ?thesis
    proof (cases "(finite V) \<and> V = V' \<and> (V \<noteq> {} \<or> A = A')")
      ...
      moreover have "V = {} \<Longrightarrow> ?rn_V = {}"
        using bij
        by simp
      ultimately have inf_dist_rename: 
    ...
    next
      ...
      hence "permute_list ?perm (to_list ?rn_V ?rn_p) 
        ...
        using perm_funs_coincide
              permute_invariant_under_coinciding_funs
                [of "(to_list ?rn_V ?rn_p)" ?perm ?perm_total]
        by presburger
      ...
      hence "permute_list ?perm (to_list ?rn_V' ?rn_p')
              = permute_list ?perm_total (to_list ?rn_V' ?rn_p')"
        using perm_funs_coincide lengths_eq
              permute_invariant_under_coinciding_funs
                [of "(to_list ?rn_V' ?rn_p')" ?perm ?perm_total]
        by fastforce
      hence rn_list_perm_list_V': 
      ...
      proof -
        ...
        have "\<forall>v' \<in> (\<pi> ` V). (card ({v\<in>(\<pi> ` V). v < v'})) < card (\<pi> ` V)"
          by (metis (no_types, lifting) card_seteq True finite_imageI less_irrefl 
        ...
        hence "?perm_total ` {0..<?len} = {0..<?len}" 
          using inj
          by (meson card_image card_subset_eq finite_atLeastLessThan)
      ...
      also have 
        "... = n (map2 (\<lambda> q q'. d (A, q) (A', q')) 
                        (permute_list ?perm_total (to_list ?rn_V ?rn_p)) 
                        (permute_list ?perm_total (to_list ?rn_V' ?rn_p')))"
        using perm \<open>symmetry n\<close> rn_lengths_eq len_V_rn_V_eq
              symmetric_norm_inv_under_map2_permute
                [of ?perm_total "to_list ?rn_V ?rn_p" "to_list ?rn_V' ?rn_p'" "n" "d" "A" "A'"]
        by fastforce
