...
fun closest_preimg_distance :: "('x \<Rightarrow> 'y) \<Rightarrow> 'x set \<Rightarrow> 'x Distance
...
fun minimizer :: "('x \<Rightarrow> 'y) \<Rightarrow> 'x set \<Rightarrow> 'x Distance \<Rightarrow> 'y set \<Rightarrow> 'x \<Rightarrow> 'y set" where
  "minimizer f domain\<^sub>f d Y x =
      arg_min_set (closest_preimg_distance f domain\<^sub>f d x) Y"
...
proof -
  ...
  moreover have "group_action G UNIV ?\<psi>"
    ...
    by blast
  moreover have
    "is_symmetry ?img (action_induced_equivariance (carrier G) X \<phi> (set_action ?\<psi>))"
...
proof
  ...
  also have
    "\<dots> = singleton_set_system
            (arg_min_set (score d C E) (limit_set (alternatives_\<E> E) UNIV))"
  proof (safe)
    fix R :: "'r set"
    assume
    ...
    then obtain r :: "'r" where
      ...
      by auto
    have "\<nexists> R'. R' \<in> singleton_set_system (limit_set (alternatives_\<E> E) UNIV)
    ...
    hence "\<nexists> r'. r' \<in> limit_set (alternatives_\<E> E) UNIV
        \<and> closest_preimg_distance
              (elect_r \<circ> fun\<^sub>\<E> (rule_\<K> C)) (elections_\<K> C) d E {r'}
          < closest_preimg_distance
              (elect_r \<circ> fun\<^sub>\<E> (rule_\<K> C)) (elections_\<K> C) d E {r}"
      ...
      by auto
    hence
      "\<nexists> r'. r' \<in> limit_set (alternatives_\<E> E) UNIV
  ...
  next
    ...
    then obtain r :: "'r" where
      res_singleton: "R = {r}" and
      r_min_lim_set:
        "r \<in> arg_min_set (score d C E) (limit_set (alternatives_\<E> E) UNIV)"
      by auto
    hence "\<nexists> r'. r' \<in> limit_set (alternatives_\<E> E) UNIV
                \<and> score d C E r' < score d C E r"
    ...
    hence
      ...
      by metis
    moreover have
      "\<forall> R' \<in> singleton_set_system (limit_set (alternatives_\<E> E) UNIV).
            \<exists> r' \<in> limit_set (alternatives_\<E> E) UNIV. R' = {r'}"
      by auto
    ultimately have
      "\<nexists> R'. R' \<in> singleton_set_system (limit_set (alternatives_\<E> E) UNIV)
          \<and> closest_preimg_distance
                (elect_r \<circ> fun\<^sub>\<E> (rule_\<K> C)) (elections_\<K> C) d E R'
            < closest_preimg_distance
                (elect_r \<circ> fun\<^sub>\<E> (rule_\<K> C)) (elections_\<K> C) d E R"
      ...
      by auto
    moreover have
      "R \<in> singleton_set_system (limit_set (alternatives_\<E> E) UNIV)"
...
theorem (in result) invar_dist_cons_imp_invar_dr_rule:
  ...
  assumes
    ...
    invar_res:
      "is_symmetry (\<lambda> E. limit_set (alternatives_\<E> E) UNIV) (Invariance rel)" and
...
proof -
  ...
  moreover have
    "\<forall> E g. g \<in> carrier G \<longrightarrow> E \<in> B \<longrightarrow>
        limit_set (alternatives_\<E> (\<phi> g E)) UNIV =
              \<psi> g ` (limit_set (alternatives_\<E> E) UNIV)"
  ...
  ultimately have "\<forall> E g. g \<in> carrier G \<longrightarrow> E \<in> B \<longrightarrow>
    ...
    by simp
  moreover have
    "\<forall> E g. {{r} | r. r \<in> \<psi> g ` (limit_set (alternatives_\<E> E) UNIV)} =
          {\<psi> g ` {r} | r. r \<in> limit_set (alternatives_\<E> E) UNIV}"
    by blast
  moreover have
    "\<forall> E g. {\<psi> g ` {r} | r. r \<in> limit_set (alternatives_\<E> E) UNIV} =
          ?\<psi>' g {{r} | r. r \<in> limit_set (alternatives_\<E> E) UNIV}"
    unfolding set_action.simps
    by blast
  ultimately have
    "is_symmetry (\<lambda> E. singleton_set_system (limit_set (alternatives_\<E> E) UNIV))
...
theorem (in result) anon_dist_and_cons_imp_anon_dr:
  ...
  assumes
    ...
    closed_C: "closed_restricted_rel (anonymity\<^sub>\<R> valid_elections)
                  valid_elections (elections_\<K> C)"
    shows "anonymity' valid_elections (distance_\<R> d C)"
proof -
  have "\<forall> \<pi>. \<forall> E \<in> elections_\<K> C.
      \<phi>_anon (elections_\<K> C) \<pi> E = \<phi>_anon valid_elections \<pi> E"
  ...
  thus ?thesis
    using cons_domain_valid assms anonymous_group_action.group_action_axioms
          well_formed_res_anon invar_dist_cons_imp_invar_dr_rule
...
theorem (in result_properties) neutr_dist_and_cons_imp_neutr_dr:
  ...
  assumes
    ...
    closed_C: "closed_restricted_rel (neutrality\<^sub>\<R> valid_elections)
                  valid_elections (elections_\<K> C)"
  shows "neutrality valid_elections (distance_\<R> d C)"
proof -
  have "\<forall> \<pi>. \<forall> E \<in> elections_\<K> C.
      \<phi>_neutr valid_elections \<pi> E = \<phi>_neutr (elections_\<K> C) \<pi> E"
    using cons_domain_valid extensional_continuation_subset
  ...
  hence "is_symmetry (elect_r \<circ> fun\<^sub>\<E> (rule_\<K> C))
    ...
    using neutr_C equivar_ind_by_act_coincide
    unfolding consensus_rule_neutrality.simps
    by (metis (no_types, lifting))
  thus ?thesis
    using neutr_d closed_C \<phi>_neutral_action.group_action_axioms
          well_formed_res_neutr act_neutr cons_domain_valid[of C]
          invar_dist_equivar_cons_imp_equivar_dr_rule[of
            _ _ "\<phi>_neutr valid_elections"]
    by simp
qed
...
theorem reversal_sym_dist_and_cons_imp_reversal_sym_dr:
  ...
  assumes
    ...
    closed_C: "closed_restricted_rel (reversal\<^sub>\<R> valid_elections)
                  valid_elections (elections_\<K> C)"
  shows "reversal_symmetry valid_elections (\<S>\<W>\<F>_result.distance_\<R> d C)"
proof -
  have "\<forall> \<pi>. \<forall> E \<in> elections_\<K> C.
      \<phi>_rev valid_elections \<pi> E = \<phi>_rev (elections_\<K> C) \<pi> E"
    using cons_domain_valid extensional_continuation_subset
