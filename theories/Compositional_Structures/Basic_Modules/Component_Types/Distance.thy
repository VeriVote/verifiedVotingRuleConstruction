...
proof (simp only: rewrite_invariant_dist, safe)
  fix
    ...
    q' :: "'a rel"
  assume
    "\<pi> \<in> carrier neutrality\<^sub>\<G>"
  hence bij: "bij \<pi>"
  ...
  proof (cases "A = A'")
    ...
    proof (unfold bij_betw_def inj_on_def, standard, standard, standard, standard)
      fix
        x :: "'a \<times> 'a" and y :: "'a \<times> 'a"
      assume
        "x \<in> ?swap_set" and "y \<in> ?swap_set" and "?f x = ?f y"
  ...
  next
    ...
    hence "\<pi> ` A \<noteq> \<pi> ` A'"
      using bij
      by (simp add: bij_is_inj inj_image_eq_iff)
