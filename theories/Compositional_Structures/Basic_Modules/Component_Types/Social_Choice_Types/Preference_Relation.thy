...
lemma limit_presv_above:
  ...
  using assms pref_imp_in_above limit_presv_prefs
  by metis
...
proof -
  ...
  proof (induction n arbitrary: A r)
    ...
    proof (clarify)
      ...
      hence "a \<in> A' \<and> above r' a = {a}"
        using above_def lin_ord_r connex_imp_refl above_refl lin_ord_imp_connex
              refl_on_domain
        by fastforce
  ...
  next
    ...
    proof (clarify)
      ...
      then obtain B where
        ...
        using Suc_inject add_Suc card.insert_remove finite.cases insert_Diff_single
              subset_insertI
      ...
      have "\<exists> a' \<in> B. above (limit B r') a' = {a'}"
        using subset_B_card Suc.IH add_diff_cancel_left' lin_ord_r card_eq_0_iff diff_le_self
              leD lessI limit_presv_lin_ord
      ...
      next
        ...
        hence "\<forall> a' \<in> B. b \<in> above r' a'"
          using limit_rel_presv_prefs pref_imp_in_above singletonD mem_Collect_eq
  ...
  hence "\<exists> a. a \<in> A \<and> above r a = {a}"
    using fin_A non_empty_A lin_ord_r len_n_plus_one
    by blast
  thus ?thesis
...
proof -
  ...
  also have "b \<preceq>\<^sub>r b"
    ...
    by metis
  moreover have
    "\<exists> a' \<in> A. above r a' = {a'} \<and> (\<forall> a'' \<in> A. above r a'' = {a''} \<longrightarrow> a'' = a')"
...
proof -
  from lin_ord
  have "refl_on A r"
    using linear_order_on_def partial_order_onD
    by blast
  moreover from assms
  have "a \<in> A"
...
theorem above_rank:
  ...
  using assms above_one_imp_rank_one rank_one_imp_above_one
  by metis
...
proof (safe)
  ...
  have "\<forall> a' b' c'. (a', b') \<in> r \<longrightarrow> (b', c') \<in> r \<longrightarrow> (a', c') \<in> r"
    ...
    by metis
  hence "(b, c) \<in> r'"
    using b_in_A b_neq_a b_pref_a_rel c_eq_r_s_exc_a equiv_r_s_exc_a equiv_r_s_exc_a_rel
          insertE insert_Diff
    unfolding equiv_rel_except_a_def
    by metis
  hence "(a, c) \<in> r'"
...
proof (safe, simp)
  ...
  hence "(b \<in> above r a') = (b \<in> above r' a')"
    using lifted_a b_not_in_above_s lifted_mono limited_dest lifted_def lin_ord_imp_connex
...
proof -
  ...
  hence eql_rs:
    ...
    using DiffD1 limit_presv_prefs limit_rel_presv_prefs
    by simp
  ...
  proof (cases)
    ...
    proof (cases)
      ...
      hence "\<exists> a' \<in> A - {a}.
                (let q = limit A r in a \<preceq>\<^sub>q a') \<and> (let u = limit A r' in a' \<preceq>\<^sub>u a)"
        using DiffD1 limit_presv_prefs a_in_A
        by simp
      thus ?thesis
    ...
    next
      ...
      moreover have not_worse: "\<forall> a' \<in> A - {a}. \<not> (a' \<preceq>\<^sub>r a \<and> a \<preceq>\<^sub>r' a')"
        using lifted subset lifted_imp_switched
        by fastforce
      ...
      have "\<forall> a' \<in> A - {a}. ((a, a') \<in> (limit A r)) = ((a, a') \<in> (limit A r'))"
        using DiffD1 limit_imp_rel limit_rel_r limit_rel_s a_in_A
              strict_pref_to_a not_worse
        by metis
      hence
        "\<forall> a' \<in> A - {a}.
      ...
      moreover have
        ...
        using a_in_A strict_pref_to_a not_worse DiffD1 limit_rel_presv_prefs
              limit_rel_s limit_rel_r
        by metis
...
theorem lifted_above_winner_single:
  ...
  assumes
    ...
    "finite A"
  shows "above r' a = {a}"
  using assms lifted_above_winner_alts
  by metis
...
proof (rule ccontr)
  ...
  hence "above r' b = {b} \<or> above r' a = {a}"
    using lifted_a fin_A lifted_above_winner_alts
    by metis
  moreover have "\<forall> a''. above r' a'' = {a''} \<longrightarrow> a'' = a'"
    using all_not_in_conv lifted_a a'_above_a' fin_A above_one_eq
    unfolding lifted_def equiv_rel_except_a_def
    by metis
  ultimately have "b = a'"
    using a_not_a'
