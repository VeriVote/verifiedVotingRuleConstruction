      ...
      { thus ?case
        proof cases
          { assume card_unchanged: "card (defer (acc \<triangleright> m) A p) = card (defer acc A p)"
            ...
            thus ?thesis
              using monotone_m defer_lift_invariant_seq
              by blast
          }
        next
          { assume card_changed: "\<not> (card (defer (acc \<triangleright> m) A p) = card (defer acc A p))"
            ...
            have card_smaller_for_p: "electoral_module (acc) \<longrightarrow>
              ...
              by (metis (full_types) monotone_m order.not_eq_order_implies_strict
                    defer_lift_invariant_def)
                ...
                proof cases
                  { assume t_not_satisfied_for_p: "\<not> t (acc A p)"
                    hence t_not_satisfied_for_q: " defer_lift_invariant (acc) \<longrightarrow>
                      ...
                      by (smt defer_from_input monotone_m prod.sel(2) profile seq_comp.simps
                          seq_n_input_sound defer_lift_invariant_def subsetCE)
                    ...
                    have "(defer_lift_invariant (acc \<triangleright> m) \<and> defer_lift_invariant (acc)) \<longrightarrow>
                      ...
                      by (smt defer_from_input monotone_m profile seq_comp.simps seq_n_input_sound
                          snd_conv defer_lift_invariant_def subsetCE)
                    hence "defer_lift_invariant (acc \<triangleright> m) \<and> defer_lift_invariant (acc) \<longrightarrow>
                      ...
                      by (smt defer_card_acc defer_from_input monotone_m prod.sel(2) profile
                          psubsetI seq_comp.simps seq_n_input_sound defer_lift_invariant_def
                          subsetCE)
                    from this t_not_satisfied_for_p
                    have rec_step_q:
                        "(defer_lift_invariant (acc \<triangleright> m) \<and> defer_lift_invariant (acc)) \<longrightarrow>
                                (\<forall>q a. (a \<in> (defer (acc \<triangleright> m) A p) \<and> lifted A p q a) \<longrightarrow>
                                     loop_comp_helper acc m t A q
                                        = loop_comp_helper (acc \<triangleright> m) m t A q)"
                      by (smt defer_from_input loop_comp_helper.simps(2) monotone_m prod.sel(2)
                          profile seq_comp.simps seq_n_input_sound defer_lift_invariant_def
                          subsetCE)
                    have rec_step_p: "electoral_module acc \<longrightarrow>
                                  loop_comp_helper acc m t A p = loop_comp_helper (acc \<triangleright> m) m t A p"
                      by (smt card_changed defer_from_input loop_comp_helper.simps(2)
                          monotone_m prod.sel(2) profile psubsetI seq_comp.simps seq_n_input_sound
                          defer_lift_invariant_def t_not_satisfied_for_p)
                    thus ?thesis
                      by (smt card_smaller_for_p less.hyps loop_comp_helper_does_not_add_to_defer
                          monotone_m defer_lift_invariant_seq profile rec_step_q
                          defer_lift_invariant_def subsetCE)
                  }
                next
...
lemma loop_comp_helper_defer_lift_invariant:
  ...
  shows "\<forall>q a. (lifted A p q a \<and> a \<in> (defer (loop_comp_helper acc m t) A p)) \<longrightarrow>
               (loop_comp_helper acc m t) A p = (loop_comp_helper acc m t) A q"
  using loop_comp_helper_defer_lift_invariant_helper monotone_m monotone_acc profile
  by blast
...
lemma loop_comp_helper_defer_lift_invariant2:
  ...
  shows "\<forall>A p q a. (finite_profile A p \<and>
                    ...
                    a \<in> (defer (loop_comp_helper acc m t) A p)) \<longrightarrow>
                      (loop_comp_helper acc m t) A p = (loop_comp_helper acc m t) A q"
  using loop_comp_helper_defer_lift_invariant monotone_acc monotone_m
  by blast
...
proof (induct n arbitrary: acc rule: less_induct)
  case(less n)
    { have subset: "(card (defer acc A p) > 1 \<and> finite_profile A p \<and> electoral_module acc) \<longrightarrow>
        ...
        using seq_comp_defer_set_single_elimination_reject single_elimination
        by blast
      hence step_reduces_defer_set: "(card (defer acc A p) > 1 \<and> finite_profile A p
                                        \<and> non_electing acc)
        ...
        next
          { assume term_not_satisfied: "\<not>(t (acc A p))"
            ...
            have rec_step: "(card (defer acc A p) > 1 \<and> finite_profile A p \<and> non_electing acc) \<longrightarrow>
              ...
              by (metis loop_comp_helper.simps(2) non_electing_def seq_n_input_sound
                ...
                next
                  { assume old_card_at_least_x: "\<not>(card (defer acc A p) < x)"
                    ...
                    hence step_profile: "electoral_module acc \<longrightarrow>
                      ...
                      using profile limit_profile_consistent
                      by auto
                    hence "electoral_module acc \<longrightarrow>
                      ...
                      using non_electing_m single_elimination
                            single_elimination_reduces_defer_set_by_one2 enough_leftover
                      by blast
                    ...
                    have "electoral_module acc
                          \<and> electoral_module m \<longrightarrow> defer (acc \<triangleright> m) A p \<subseteq> defer acc A p"
                      using enough_leftover profile subset
                      by blast
                      ...
                      { thus ?thesis
                        proof cases
                          { assume new_card_greater_x: "electoral_module acc \<longrightarrow> i > x"
                            ...
                            moreover have new_card_still_big_enough2:
                                            "electoral_module acc \<longrightarrow> x \<le> i" (*Need this for step*)
                            ...
                            moreover have "n = card (defer acc A p) \<longrightarrow>
                              ...
                              using subset step_profile enough_leftover profile psubset_card_mono
                                i_is_new_card
                              by blast
                            moreover have "electoral_module acc \<longrightarrow> electoral_module (acc \<triangleright> m)"
                                          (*Need this for step*)
                              using non_electing_def non_electing_m seq_creates_modules
                              by blast
                            moreover have non_electing_new:
                              "non_electing acc \<longrightarrow> non_electing (acc \<triangleright> m)"
                              by (simp add: non_electing_m)
                            ultimately have "(n = card (defer acc A p) \<and> non_electing acc
                                        \<and> electoral_module acc)
                                          \<longrightarrow> card (defer (loop_comp_helper (acc \<triangleright> m) m t) A p) = x"
                              using less.hyps i_is_new_card new_card_greater_x
                              by blast
                            thus ?thesis
                              using profile rec_step
                        ...
                        next
                          { assume i_not_gt_x: "\<not>(electoral_module acc \<longrightarrow> i > x)"
                            ...
                            thus ?thesis
                              by (metis i_not_gt_x dual_order.strict_iff_order i_is_new_card
                                  loop_comp_helper.simps(1) new_card_still_big_enough profile
                                  rec_step terminate_if_n_left)
                          }
                        qed }
                    }
                  qed }
      ...
      next
        { assume "\<not>card A < x"
          ...
          hence card_m: "card (defer m A p) = card A - 1"
            using non_electing_m profile single_elimination
                  single_elimination_reduces_defer_set_by_one2 x_greater_zero
            by fastforce
          hence card_big_enough_m: "card (defer m A p) \<ge> x"
            ...
            thus ?thesis
              by (metis (no_types, hide_lams) card_big_enough_m
                  loop_comp_helper_iterative_elimination_number_of_survivors_for_eliminates
                  non_electing_m profile single_elimination terminate_if_n_left x_greater_zero)
          }
    qed }
qed
