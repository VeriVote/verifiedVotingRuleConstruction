...
*)
fun disjoint3 :: "'a Result \<Rightarrow> bool" where "disjoint3 (e, r, d) =
  ((e \<inter> r = {}) \<and>
   (e \<inter> d = {}) \<and>
   (r \<inter> d = {}))"
fun unify_to :: "'a set \<Rightarrow>'a Result \<Rightarrow> bool" where "unify_to A (e, r, d) \<longleftrightarrow> (e \<union> r \<union> d = A)"
...
*)
definition electoral_module :: " 'a Electoral_module \<Rightarrow> bool" where "electoral_module m \<equiv>
...
lemma electoral_module_intro[intro]:
  ...
  unfolding electoral_module_def by auto
...
abbreviation elect_r :: "'a Result \<Rightarrow> 'a set" where "elect_r r \<equiv> fst r"
abbreviation reject_r :: "'a Result \<Rightarrow> 'a set" where "reject_r r \<equiv> fst (snd r)"
abbreviation defer_r :: "'a Result \<Rightarrow> 'a set" where "defer_r r \<equiv> snd (snd r)"
...
proof - (* autogenerated proof *)
  have f1: "\<forall>A Aa. (Aa::'a set) - (A - Aa) = Aa"
    by blast
  then have f2: "\<forall>A Aa Ab. (Ab::'a set) - (Aa - Ab \<union> A) = Ab \<inter> (Ab - A)"
    by (metis Diff_Un)
  ...
  { assume "\<exists>A. r \<union> A - d \<noteq> r \<union> (A - d)"
    then have "r - d \<noteq> r"
      by (metis Un_Diff)
    then have "\<not> disjoint3 (e, r, d)"
  ...
  { assume "\<exists>Aa. r \<union> d - Aa \<noteq> A - (e \<union> Aa)"
    { assume "\<exists>A. A \<union> d - e \<noteq> A - e \<union> d"
      then have "d - e \<noteq> d"
        by (metis Un_Diff)
      then have "\<not> disjoint3 (e, r, d)"
        using f1 by (metis Un_Diff_Int disjoint3.simps sup_bot.comm_neutral)
    }
    then have "disjoint3 (e, r, d) \<longrightarrow> partition_of A (e, r, d) \<longrightarrow> A - (e \<union> d) = r"
      using f4 f3 f2 f1
      by (metis Un_Diff Un_Diff_Int disjoint3.simps partition_of_def sup_bot.comm_neutral
          unify_to.simps)
  }
  ultimately show ?thesis
    using f3 by (metis (no_types) partition_of_def sup_bot.comm_neutral)
qed
...
proof -
  from module profile have "unify_to A (m A p)"
    using electoral_module_def partition_of_def by blast
...
proof -
  from module profile have 0: "partition_of A (m A p)"
    by (simp add: electoral_module_def)
  from this module profile have "(elect m A p) \<union> (reject m A p) \<union> (defer m A p) = A"
    using module partition_of_def profile split_unify_result by blast
  moreover from 0 have "(elect m A p) \<inter> (reject m A p) = {} \<and>
                        (reject m A p) \<inter> (defer m A p) = {}"
...
proof -
  ...
  hence "(elect m A p) \<union> (reject m A p) \<union> (defer m A p) = A"
    using module profile split_unify_result by auto
  moreover from 0 have "(elect m A p) \<inter> (defer m A p) = {} \<and> (reject m A p) \<inter> (defer m A p) = {}"
    using module profile split_disjoint3 by blast
  ultimately show ?thesis
    by blast
qed
...
definition non_blocking :: "'a Electoral_module \<Rightarrow> bool" where "non_blocking m \<equiv>
...
definition electing :: "'a Electoral_module \<Rightarrow> bool" where "electing m \<equiv>
...
definition non_electing :: "'a Electoral_module \<Rightarrow> bool" where "non_electing m \<equiv>
...
*)
definition defers :: "nat \<Rightarrow> 'a Electoral_module \<Rightarrow> bool" where "defers n m \<equiv>
...
*)
definition rejects :: "nat \<Rightarrow> 'a Electoral_module \<Rightarrow> bool" where "rejects n m \<equiv>
...
definition eliminates :: "nat \<Rightarrow> 'a Electoral_module \<Rightarrow> bool" where "eliminates n m \<equiv>
...
(* "elects n" is true for all electoral modules, that elect exactly n alternatives, whenever
   there are n or more alternatives.
*)
definition elects :: "nat \<Rightarrow> 'a Electoral_module \<Rightarrow> bool" where "elects n m \<equiv>
...
proof -
  ...
  proof fix i::nat show "i < size p \<longrightarrow> linear_orders.only_change A (p!i) (q!i) a"
  proof assume i_ok: "i < size p"
...
proof cases
  ...
  hence one: "\<forall>i::nat. i < size p \<longrightarrow>
               (linear_orders.lifted A (limit_to A (p!i)) (limit_to A (q!i)) a \<or>
               (limit_to A (p!i)) = (limit_to A (q!i)))"
  ...
  next
    ...
    moreover have "size ?p = size ?q"
      using electoral_modules.lifted_def lifted by fastforce
    ...
    moreover have "\<forall>i::nat. (i < size ?p \<and> \<not>linear_orders.lifted A (?p!i) (?q!i) a) \<longrightarrow>
                       (?p!i) = (?q!i)"
      by (metis electoral_modules.lifted_def length_map lifted limit_profile_to.simps nth_map one)
...
theorem strict_def_monotone_implies_def_monotone[simp]:
  ...
  using assms defer_monotone_def defer_lift_invariant_def by fastforce
