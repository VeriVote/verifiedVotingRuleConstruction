...
proof -
  { have "\<forall>A p. finite_profile A p \<longrightarrow> partition_of A ((m \<parallel>\<^sub>a n) A p)"
    proof
      { fix A show "\<forall>p. finite_profile A p \<longrightarrow> partition_of A ((m \<parallel>\<^sub>a n) A p)"
        proof
          { fix p show "finite_profile A p \<longrightarrow> partition_of A ((m \<parallel>\<^sub>a n) A p)"
            proof
              { assume profile: "finite_profile A p"
                ...
                proof -
                  ...
                  hence "\<forall>A Aa Ab Ac Ad p f.
                            partition_of (Ab::'a set) (f Ab (Ac, A, Ad) (Aa, p))
                          \<or> \<not> partition_of Ab (Ac, A, Ad)
                          \<or> \<not> partition_of Ab (Aa, p)
                          \<or> \<not> aggregator f"
                    by (metis aggregator_def)
                  hence "\<forall>A p pa f.
                            partition_of (A::'a set) (f A p pa)
                          \<or> \<not> partition_of A p
                          \<or> \<not> partition_of A pa
                          \<or> \<not> aggregator f"
                    by auto
                  thus ?thesis
  ...
  assume not_a1: "x \<notin> elect (m \<parallel>\<^sub>a n) A p"
    { thus ?thesis
      ...
      next
        ...
        from this not_a1
        have a3: "x \<in> reject (m \<parallel>\<^sub>a n) A p"
...
proof -
  { have "x \<in> reject (m \<parallel>\<^sub>a n) A p \<longrightarrow> (x \<in> reject m A p \<and> x \<in> reject n A p)"
    proof
      assume a: "x \<in> reject (m \<parallel>\<^sub>a n) A p"
      hence "let (e1, r1, d1) = m A p; (e2, r2, d2) = n A p in
              x \<in> fst (snd (Max_aggregator A (e1, r1, d1) (e2, r2, d2)))"
    ...
    qed }
  moreover
    { have "(x \<in> reject m A p \<and> x \<in> reject n A p) \<longrightarrow> x \<in> reject (m \<parallel>\<^sub>a n) A p"
      proof
        ...
        thus "x \<in> reject (m \<parallel>\<^sub>a n) A p"
          by (smt CollectD DiffD1 Max_aggregator.simps Un_iff a electoral_module_element_in_defer
              ...
              parallel_comp.simps prod.collapse prod.simps(2) profile
...
proof -
  { have "A = (elect m A p) \<union> (reject m A p) \<union> (defer m A p)
          \<and> A = (elect n A p) \<union> (reject n A p) \<union> (defer n A p)"
      by (simp add: module_m module_n profile split_unify_result)
    hence "A - ((elect m A p) \<union> (defer m A p)) = (reject m A p)
          \<and> A - ((elect n A p) \<union> (defer n A p)) = (reject n A p)"
        using module_m module_n profile reject_alternative_representation
        by auto
...
proof -
  { have "\<forall>S. finite S \<longrightarrow>
      (\<exists>A \<subseteq> S.
        (\<forall>a \<in> A. independent_of (Drop_module n r) S a
              \<and> (\<forall>p. finite_profile S p \<longrightarrow> a \<in> reject (Drop_module n r) S p))
      \<and> (\<forall>a \<in> S-A. independent_of (Pass_module n r) S a
              \<and> (\<forall>p. finite_profile S p \<longrightarrow> a \<in> reject (Pass_module n r) S p)))"
    proof
        ...
        proof
          { assume fin: "finite S"
            ...
            proof
              ...
              ultimately show
                "?A \<subseteq> S
                \<and> (\<forall>a \<in> ?A. independent_of (Drop_module n r) S a
                        \<and> (\<forall>p. finite_profile S p \<longrightarrow> a \<in> reject (Drop_module n r) S p))
                \<and> (\<forall>a \<in> S-?A. independent_of (Pass_module n r) S a
                        \<and> (\<forall>p. finite_profile S p \<longrightarrow> a \<in> reject (Pass_module n r) S p))"
                by simp
      ...
      { fix S
        ...
        proof
          { fix p
            ...
            proof
              { fix q show "\<forall> x. (x \<in> (defer (m \<parallel>\<^sub>a n) S p) \<and> lifted S p q x) \<longrightarrow>
                              (m \<parallel>\<^sub>a n) S p = (m \<parallel>\<^sub>a n) S q"
                proof
                  { fix x show "(x \<in> (defer (m \<parallel>\<^sub>a n) S p) \<and> lifted S p q x) \<longrightarrow>
                                  (m \<parallel>\<^sub>a n) S p = (m \<parallel>\<^sub>a n) S q"
                    proof
                      assume a: "x \<in> (defer (m \<parallel>\<^sub>a n) S p) \<and> lifted S p q x"
                      hence profiles: "finite_profile S p \<and> finite_profile S q"
                        by (simp add: lifted_def)
                      from compatible
                      obtain A::"'a set" where
                        A: "A \<subseteq> S
                          \<and> (\<forall>x \<in> A. independent_of m S x
                                    \<and> (\<forall>p. finite_profile S p \<longrightarrow> x \<in> reject m S p))
                          \<and> (\<forall>x \<in> S-A. independent_of n S x
                                    \<and> (\<forall>p. finite_profile S p \<longrightarrow> x \<in> reject n S p))"
                        ...
                        { have "\<forall>x \<in> S. eq_result_for (m \<parallel>\<^sub>a n) S p q x"
                          proof cases
                            { assume a0: "x \<in> A"
                              ...
                              have "\<forall>x \<in> S-A. eq_module_result (m \<parallel>\<^sub>a n) m S p x"
                                by (metis A max_agg max_reject_interaction2 monotone_m monotone_n
                                    profiles defer_lift_invariant_def)
                              moreover have "\<forall>x \<in> S. eq_result_for m S p q x"
                                by (smt A a a0 eq_result_for_def independent_of_def
                                    lifted_impl_only_change profiles)
                              moreover have "\<forall>x \<in> S-A. eq_module_result m (m \<parallel>\<^sub>a n) S q x"
                                by (metis A max_agg max_reject_interaction1 monotone_m monotone_n
                                    profiles defer_lift_invariant_def)
                          ...
                          next
                            { assume "x \<notin> A"
                              hence a1: "x \<in> S-A"
                                by (metis (no_types, lifting) DiffI a compatible defer_from_input
                                    disjoint_compatible_def max_agg max_aggregator_sound
                                    par_creates_modules profiles subsetCE)
                              hence "x \<in> reject n S p"
                                using A profiles
                                by blast
                              from this a
                              have defer_n: "x \<in> defer m S p"
                                by (metis DiffD1 DiffD2 compatible compatible_one_decides
                                    defer_alternative_representation disjoint_compatible_def
                                    electoral_module_element_in_elect_or_defer eq_module_result_def
                                    max_agg max_aggregator_sound par_creates_modules profiles)
                              have "\<forall>x \<in> A. eq_module_result (m \<parallel>\<^sub>a n) n S p x"
                                by (metis A compatible disjoint_compatible_def max_agg
                                    max_reject_interaction4 profiles)
                              moreover have "\<forall>x \<in> S. eq_result_for n S p q x"
                                by (smt A a a1 eq_result_for_def independent_of_def
                                    lifted_impl_only_change profiles)
                              moreover have "\<forall>x \<in> A. eq_module_result n (m \<parallel>\<^sub>a n) S q x"
                                by (metis A compatible disjoint_compatible_def max_agg
                                    max_reject_interaction3 profiles)
                              ...
                              have "\<forall>x \<in> S-A. eq_module_result (m \<parallel>\<^sub>a n) m S p x"
                                by (metis A max_agg max_reject_interaction2 monotone_m monotone_n
                                    profiles defer_lift_invariant_def)
                              moreover have "\<forall>x \<in> S. eq_result_for m S p q x"
                                by (smt a defer_n eq_result_for_def monotone_m profiles
                                    defer_lift_invariant_def)
                              moreover have "\<forall>x \<in> S-A. eq_module_result m (m \<parallel>\<^sub>a n) S q x"
                                by (metis A max_agg max_reject_interaction1 monotone_m monotone_n
                                    profiles defer_lift_invariant_def)
                        ...
                        thus "(m \<parallel>\<^sub>a n) S p = (m \<parallel>\<^sub>a n) S q"
                          by (metis compatible disjoint_compatible_def forall_eq_result max_agg
                              max_aggregator_sound par_creates_modules profiles)
                      qed }
                  qed }
              qed }
          qed }
...
proof -
  ...
  moreover have conservative_def_inline:
      "aggregator a \<and> (\<forall>A e1 e2 d1 d2 r1 r2.
    ...
    using conservative conservative_def
    by blast
...
proof -
  from compatible obtain A where
          A: "A \<subseteq> S
              \<and> (\<forall>a \<in> A. independent_of x S a \<and> (\<forall>p. finite_profile S p \<longrightarrow> a \<in> reject x S p))
              \<and> (\<forall>a \<in> S-A. independent_of y S a \<and> (\<forall>p. finite_profile S p \<longrightarrow> a \<in> reject y S p))"
  ...
  have reject_representation: "reject (x \<parallel>\<^sub>a y) S p = (reject x S p) \<inter> (reject y S p)"
    using max_agg max_agg_reject_diffrent_representation disjoint_compatible_def
    by blast
  ...
  hence "finite (reject x S p) \<and> finite (reject y S p)"
    using rev_finite_subset profile reject_from_input
    by auto
  ...
  from 0 1
  show "card (reject (x \<parallel>\<^sub>a y) S p) = n"
    by simp
qed
...
proof -
  { have "\<forall>A p. (card A > 1 \<and> finite_profile A p) \<longrightarrow> card (reject (m \<parallel>\<^sub>a n) A p) = 1"
    proof
      { fix A show "\<forall>p. (card A > 1 \<and> finite_profile A p) \<longrightarrow> card (reject (m \<parallel>\<^sub>a n) A p) = 1"
        proof
          { fix p show "(card A > 1 \<and> finite_profile A p) \<longrightarrow> card (reject (m \<parallel>\<^sub>a n) A p) = 1"
            proof
              { assume asm0: "card A > 1 \<and> finite_profile A p"
                ...
                moreover from card_geq_1
                have "card (defer m A p) = 1"
